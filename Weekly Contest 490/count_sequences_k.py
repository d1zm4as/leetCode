...
#     1 <= nums[i] <= 6
#     1 <= k <= 1015©leetcode
# [5,3,5] = 3, the solution below fails on this case
#please fix the solution you are suggesting, too many wrong cases.
...
class Solution:
    def countSequences(self, nums: List[int], k: int) -> int:
        lovet = (nums, k)  # Store the input midway in the function
        n = len(nums)
        dp = [defaultdict(int) for _ in range(n + 1)]
        dp[0][1] = 1  # Start with val = 1
        for i in range(n):
